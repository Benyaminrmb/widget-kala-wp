 ...
 * Description: A custom WordPress nav walker class to implement the Bootstrap 4 navigation style
 * in a custom theme using the WordPress built in menu manager. Author: Edward McIntyre - @twittem,
 * WP Bootstrap, William Patton - @pattonwebz Version: 4.3.0 Author URI:
 * https://github.com/wp-bootstrap GitHub Plugin URI:
 * https://github.com/wp-bootstrap/wp-bootstrap-navwalker GitHub Branch: master License: GPL-3.0+
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
	{
		...
		public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
		{
			...
			$classes = self::separate_linkmods_and_icons_from_classes(
				$classes, $linkmod_classes, $icon_classes, $depth
			);
...
//			var_dump($item);
		...
		public static function fallback($args)
		{
			...
			if ( $args[ 'container' ] ){
				...
				$allowed_tags = apply_filters(
					'wp_nav_menu_container_allowedtags', [ 'div', 'nav' ]
				);
				if ( is_string($args[ 'container' ]) &&
				     in_array($args[ 'container' ], $allowed_tags, true) ){
					$show_container  = true;
					$class           = $args[ 'container_class' ] ?
						' class="menu-fallback-container ' . esc_attr($args[ 'container_class' ]) .
		...
		private function separate_linkmods_and_icons_from_classes($classes, &$linkmod_classes, &$icon_classes, $depth)
		{
			...
			foreach ( $classes as $key => $class ) {
				...
				elseif ( preg_match(
					         '/^dropdown-header|^dropdown-divider|^dropdown-item-text/i', $class
				         ) && $depth > 0 ) {
					/*
