                ...
                <div class="grid gap-5 grid-cols-6">
                    <?php while (have_rows('frontpage_bottom_advertises',$page_id)): the_row();
                        ...
                        if (has_sub_field('link')) {
                            $before = '<a class="flex rounded-md overflow-hidden col-span-2" href="'.$link['url'].'" target="'.esc_attr($link['target']).'" title="'.$link['title'].'">';
                            $after  = '</a>';
                        }
            ...
            if (have_rows('frontpage_bottom_buttons', $page_id)):?>
                <div class="grid gap-5 grid-cols-6">
                    <?php while (have_rows('frontpage_bottom_buttons',$page_id)) {
                        ...
                        class="btn-custom-white col-span-1 font-light text-white"
